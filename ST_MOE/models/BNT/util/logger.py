    ...
    def evaluate(self, k=None, initialize=False, option='mean', print=True):
        ...
        if self.num_classes==1:
            if not self.k_fold is None and k is None:
                ...
                mae = aggregate([metrics.mean_absolute_error(samples['true'][k], samples['pred'][k]) for k in self.k_fold])
                corr = np.corrcoef(samples['true'], samples['pred'])[0,1]
            else:
                ...
                mae = metrics.mean_absolute_error(samples['true'], samples['pred'])
                corr = np.corrcoef(samples['true'], samples['pred'])[0,1]
            
            if initialize:
                self.initialize(k)
                
            # metric = dict(explained_var=explained_var, r2=r2, mse=mse, mae=mae)
            metric = dict(mse=mse, mae=mae, corr=corr)
