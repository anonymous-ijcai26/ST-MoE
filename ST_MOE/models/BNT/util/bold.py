...
def flatten_upper_triangle_batch(matrix_batch):

    assert matrix_batch.ndim == 3, f"期望3D张量 [batch, N, N]，实际得到 {matrix_batch.ndim}D"
    batch_size, n_rows, n_cols = matrix_batch.shape
    assert n_rows == n_cols, f"期望方阵，实际得到形状 [{n_rows}, {n_cols}]"
    

    mask = torch.triu(
        torch.ones(n_rows, n_cols, dtype=torch.bool, device=matrix_batch.device), 
        diagonal=1
    )

    flattened_batch = matrix_batch[:, mask]
