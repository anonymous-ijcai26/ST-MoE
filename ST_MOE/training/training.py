...
class EarlyStopping:
    ...
    def __call__(self, test_accuracy, model):
        ...
        if self.best_score is None:
            ...
            self.save_checkpoint(test_accuracy, model)
            ...
            self.yuan_socre = self.best_score
            self.best_score = score
            self.save_checkpoint(self.yuan_socre, model)
            self.epochs_no_improve = 0

    def save_checkpoint(self, test_accuracy, model):

        if self.verbose:
            print(f'Validation loss decreased ({self.yuan_socre:.6f} --> {self.best_score:.6f}). Saving model...')
...
class Train:
    ...
    def __init__(self, cfg: DictConfig,
        ...
        self.logger = logger
        self.model = model
        self.logger.info(f'#model params: {count_params(self.model, True)}')
    ...
    def train_per_epoch(self, optimizer, lr_scheduler):
        ...
        for time_series, d_node_feature, node_feature, label in self.train_dataloader:
            label = label.cuda() 
            self.current_step += 1
            ...
            predict = self.model(time_series, d_node_feature, node_feature)
            b = predict.size(0)
            t = predict.size(1)
            c = predict.size(2)

            predict = predict.view(b * t, c)  # 
            label = torch.argmax(label, dim=1)  # [8]

            label = label.unsqueeze(1).repeat(1, t)  # [8, 6]
    ...
    def test_per_epoch(self, dataloader, loss_meter, acc_meter):
        ...
        for time_series, d_node_feature, node_feature, label in dataloader:

            time_series, node_feature, label = time_series, node_feature, label.cuda()
            predict = self.model(time_series, d_node_feature, node_feature)
    ...
    def train(self):
        training_process = []
        early_stopping = EarlyStopping(patience=20, verbose=True, path=self.save_path / "best_model.pt")  
        self.current_step = 0
        best_val_auc = 0
        best_val_acc = None
        best_epoch = 0
        ...
        for epoch in range(self.epochs): 
            ...
            if self.test_accuracy.avg > best_test_accuracy:
                ...
                best_test_result = {
                    ...
                    'Best Test Sensitivity': test_result[-1],
                    'Best Test Specificity': test_result[-2]
                }
            ...
            early_stopping(self.test_accuracy.avg, self.model)
