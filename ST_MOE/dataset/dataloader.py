...
def init_stratified_dataloader(cfg: DictConfig,
    ...
    for train_index, test_valid_index in split.split(final_timeseires, stratified):
        final_timeseires_train, final_pearson_train, labels_train, d_final_pearson_train = final_timeseires[
            train_index], final_pearson[train_index], labels[train_index], d_final_pearson[train_index]
        final_timeseires_val_test, final_pearson_val_test, labels_val_test, d_final_pearson_val_test = final_timeseires[
            test_valid_index], final_pearson[test_valid_index], labels[test_valid_index], d_final_pearson[test_valid_index]
    ...
    for test_index, valid_index in split2.split(final_timeseires_val_test, stratified):
        final_timeseires_test, final_pearson_test, labels_test, d_final_pearson_test = final_timeseires_val_test[
            test_index], final_pearson_val_test[test_index], labels_val_test[test_index], d_final_pearson_val_test[test_index]
        final_timeseires_val, final_pearson_val, labels_val,d_final_pearson_val  = final_timeseires_val_test[
            valid_index], final_pearson_val_test[valid_index], labels_val_test[valid_index], d_final_pearson_val_test[valid_index]

    train_dataset = utils.TensorDataset(
        final_timeseires_train,
        final_pearson_train,
        d_final_pearson_train,
        labels_train
    )

    val_dataset = utils.TensorDataset(
        final_timeseires_val, final_pearson_val, d_final_pearson_val, labels_val
    )

    test_dataset = utils.TensorDataset(
        final_timeseires_test, final_pearson_test, d_final_pearson_test, labels_test
    )
