...
# 通过混合数据和标签生成新的训练样本，旨在增强模型的泛化能力和鲁棒性。
def continus_mixup_data(*xs, y=None, alpha=1.0, device='cpu'):
    '''Returns mixed inputs, pairs of targets, and lambda'''
    if alpha > 0:
        # 生成一个服从 Beta 分布的随机数lam。lam是控制混合比例的系数。
        lam = np.random.beta(alpha, alpha)
    else:
        lam = 1
    batch_size = y.size()[0]
    # 生成一个随机排列的索引，这个索引用来打乱批次中的样本顺序，以便进行混合
    index = torch.randperm(batch_size).to(device)
    # 于输入数据 xs 中的每个张量，按照 lam 和 (1-lam) 的比例，将当前批次的样本与打乱后的样本进行线性组合，生成新的输入数据 new_xs。
